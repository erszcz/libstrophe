 ...
 */
#[no_mangle]
pub unsafe extern "C" fn xmpp_jid_new(mut ctx: *mut xmpp_ctx_t,
                                      mut node: *const libc::c_char,
                                      mut domain: *const libc::c_char,
                                      mut resource: *const libc::c_char)
 -> *mut libc::c_char {
    ...
    if domain.is_null() { return 0 as *mut libc::c_char }
    /* accumulate lengths */
    dlen = strlen(domain);
    nlen =
        if !node.is_null() {
            strlen(node).wrapping_add(1 as libc::c_int as libc::c_ulong)
        } else { 0 as libc::c_int as libc::c_ulong };
    rlen =
        if !resource.is_null() {
            strlen(resource).wrapping_add(1 as libc::c_int as libc::c_ulong)
        } else { 0 as libc::c_int as libc::c_ulong };
    len = nlen.wrapping_add(dlen).wrapping_add(rlen);
    /* concat components */
    result =
    ...
    if !result.is_null() {
        if !node.is_null() {
            memcpy(result as *mut libc::c_void, node as *const libc::c_void,
                   nlen.wrapping_sub(1 as libc::c_int as libc::c_ulong));
            *result.offset(nlen.wrapping_sub(1 as libc::c_int as
                                                 libc::c_ulong) as isize) =
                '@' as i32 as libc::c_char
        }
        memcpy(result.offset(nlen as isize) as *mut libc::c_void,
               domain as *const libc::c_void, dlen);
        if !resource.is_null() {
            *result.offset(nlen.wrapping_add(dlen) as isize) =
                '/' as i32 as libc::c_char;
            memcpy(result.offset(nlen as
                                     isize).offset(dlen as
                                                       isize).offset(1 as
                                                                         libc::c_int
                                                                         as
                                                                         isize)
                       as *mut libc::c_void, resource as *const libc::c_void,
                   rlen.wrapping_sub(1 as libc::c_int as libc::c_ulong));
        }
 ...
 */
#[no_mangle]
pub unsafe extern "C" fn xmpp_jid_bare(mut ctx: *mut xmpp_ctx_t,
                                       mut jid: *const libc::c_char)
 -> *mut libc::c_char {
    let mut result: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut len: size_t = 0;
    len = strcspn(jid, b"/\x00" as *const u8 as *const libc::c_char);
    result =
        xmpp_alloc(ctx, len.wrapping_add(1 as libc::c_int as libc::c_ulong))
            as *mut libc::c_char;
    if !result.is_null() {
        memcpy(result as *mut libc::c_void, jid as *const libc::c_void, len);
        *result.offset(len as isize) = '\u{0}' as i32 as libc::c_char
    }
    return result;
}
 ...
 */
#[no_mangle]
pub unsafe extern "C" fn xmpp_jid_node(mut ctx: *mut xmpp_ctx_t,
                                       mut jid: *const libc::c_char)
 -> *mut libc::c_char {
    let mut result: *mut libc::c_char = 0 as *mut libc::c_char;
    ...
    if !c.is_null() {
        result =
            xmpp_alloc(ctx,
                       (c.wrapping_offset_from(jid) as libc::c_long +
                            1 as libc::c_int as libc::c_long) as size_t) as
                *mut libc::c_char;
        if !result.is_null() {
            memcpy(result as *mut libc::c_void, jid as *const libc::c_void,
                   c.wrapping_offset_from(jid) as libc::c_long as
                       libc::c_ulong);
            *result.offset(c.wrapping_offset_from(jid) as libc::c_long as
 ...
 */
#[no_mangle]
pub unsafe extern "C" fn xmpp_jid_domain(mut ctx: *mut xmpp_ctx_t,
                                         mut jid: *const libc::c_char)
 -> *mut libc::c_char {
    let mut result: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut c: *const libc::c_char = 0 as *const libc::c_char;
    ...
    dlen = strcspn(c, b"/\x00" as *const u8 as *const libc::c_char);
    result =
        xmpp_alloc(ctx, dlen.wrapping_add(1 as libc::c_int as libc::c_ulong))
            as *mut libc::c_char;
    if !result.is_null() {
        memcpy(result as *mut libc::c_void, c as *const libc::c_void, dlen);
        *result.offset(dlen as isize) = '\u{0}' as i32 as libc::c_char
    }
    return result;
}
 ...
 */
#[no_mangle]
pub unsafe extern "C" fn xmpp_jid_resource(mut ctx: *mut xmpp_ctx_t,
                                           mut jid: *const libc::c_char)
 -> *mut libc::c_char {
    let mut c: *const libc::c_char = 0 as *const libc::c_char;
    c = strchr(jid, '/' as i32);
    return if !c.is_null() {
               xmpp_strdup(ctx, c.offset(1 as libc::c_int as isize))
